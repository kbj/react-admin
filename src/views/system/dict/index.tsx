...
const Dict: FC<PropsWithChildren> = () => {
  ...
  const commonStatus = useDict('sys_common_status')
  // 弹窗
  ...
  const queryConfig: TableSearchFormItem[] = [
    ...
    },
    {
      ...
      label: '状态',
      fieldName: 'enabled',
      list: commonStatus.map((item) => ({ label: item.dictLabel, value: item.dictValue }))
    }
  ]
  ...
  const tableConfig: ColumnsType<IDictList> = [
    ...
    },
    {
      ...
      dataIndex: 'enabled',
      align: 'center',
      render: (text) => <Switch checkedChildren="正常" disabled unCheckedChildren="停用" checked={text} />
    },
  ...
  // 弹窗
  const dialog = (
    <Form name="form" form={form} autoComplete="off" initialValues={{ enabled: true }} labelCol={{ span: 4 }}>
